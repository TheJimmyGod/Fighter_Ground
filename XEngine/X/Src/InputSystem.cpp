...
void InputSystem::Initialize(HWND window)
{
	...
	XASSERT(SUCCEEDED(hr), "[InputSystem] Failed to create keyboard device.");

	// Set the keyboard data format
	hr = mKeyboardDevice->SetDataFormat(&c_dfDIKeyboard);
	XASSERT(SUCCEEDED(hr), "[InputSystem] Failed to set keyboard data format.");
	...
	XASSERT(SUCCEEDED(hr), "[InputSystem] Failed to set keyboard cooperative level.");

	// Acquire the keyboard device
	hr = mKeyboardDevice->Acquire();
	XASSERT(SUCCEEDED(hr), "[InputSystem] Failed to acquire keyboard device.");
	//----------------------------------------------------------------------------------------------------

	//----------------------------------------------------------------------------------------------------
	// Create mouse device
	hr = mDirectInput->CreateDevice(GUID_SysMouse, &mMouseDevice, nullptr);
	XASSERT(SUCCEEDED(hr), "[InputSystem] Failed to create mouse device.");

	// Set the mouse data format
	hr = mMouseDevice->SetDataFormat(&c_dfDIMouse);
	XASSERT(SUCCEEDED(hr), "[InputSystem] Failed to set mouse data format.");

	// Set the mouse cooperative level
	hr = mMouseDevice->SetCooperativeLevel(window, DISCL_FOREGROUND | DISCL_EXCLUSIVE);
	XASSERT(SUCCEEDED(hr), "[InputSystem] Failed to set mouse cooperative level.");

	// Acquire the mouse device
	hr = mMouseDevice->Acquire();
	XASSERT(SUCCEEDED(hr), "[InputSystem] Failed to acquire mouse device.");
...
void InputSystem::UpdateKeyboard()
{
	...
	if (FAILED(hr))
	{
		...
		if (DIERR_INPUTLOST == hr || DIERR_NOTACQUIRED == hr)
		{
			if (sWriteToLog)
			{
				XLOG("[InputSystem] Keyboard device is lost.");
				sWriteToLog = false;
			}
		...
		{
			XLOG("[InputSystem] Failed to get keyboard state.");
			return;
		}
	}
	else
...
void InputSystem::UpdateMouse()
{
	...
	if (FAILED(hr))
	{
		...
		if (DIERR_INPUTLOST == hr || DIERR_NOTACQUIRED == hr)
		{
			if (sWriteToLog)
			{
				XLOG("[InputSystem] Mouse device is lost.");
				sWriteToLog = false;
			}
		...
		{
			XLOG("[InputSystem] Failed to get mouse state.");
			return;
		}
	}
	else
