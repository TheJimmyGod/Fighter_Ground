...
namespace
{
	...
	std::vector<AudioCommand> myAudioCommands;
...
void Start(const char* appName, HINSTANCE instance, int width, int height, const int audioEngineType)
{
	CoInitialize(nullptr);
	CoInitializeEx(nullptr, COINIT_MULTITHREADED);		// for DirectX::AudioEngine
	CoInitializeEx(nullptr, COINIT_APARTMENTTHREADED);	// for FMOD::Studio::System
	...
	AudioManager::StaticInitialize((Audio::AudioEngineType)audioEngineType);
...
void Run(bool (*GameLoop)(float))
{
	...
	while (WM_QUIT != msg.message)
	{
		...
		else
		{
			...
			// Audio
			for (auto& command : myAudioCommands)
			{
				AudioSource* source = AudioManager::Get()->GetAudio(command.audioId);
				if (source)
				{
					source->Play(command.loop);
				}
			}
			myAudioCommands.clear();
...
{
	return AudioManager::Get()->LoadFromFile(filename);
}

//----------------------------------------------------------------------------------------------------

void PlayAudio(int audioId, bool loop, float volume, float pitch, float pan)
{
	AudioCommand audio;
	...
	myAudioCommands.emplace_back(audio);
}
//----------------------------------------------------------------------------------------------------
